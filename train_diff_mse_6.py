...
def train_pcpnet(opt):
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, (data, norms, init, diff) in enumerate(pbar, 0):
            data = data.float()
            # inputs = torch.cat((data[:,:,:,:3], data[:,:,:,3:6]), dim=3)
            inputs = data[:,:,:,6:9]
            # pdb.set_trace()
            ...
            ori_rmse = rms_angular_error(init, norms)
            ori_tr.append(ori_rmse)

            dot = torch.abs(torch.sum(init * norms, dim=1)).mean()

            pbar.set_postfix(Epoch=epoch, tr_loss=loss.item(), rmse_esti_ori=rmse, rmse_init_ori=ori_rmse, dot=dot)
        ...
        with torch.no_grad():
            ...
            for i, (data, norms, init, diff) in enumerate(pbar1, 0):
                ...
                ori_rmse = rms_angular_error(init, norms)
                ori_val.append(ori_rmse)
                
                dot = torch.abs(torch.sum(init * norms, dim=1)).mean()

                # if (i%50) == 0:
                # print(f'epoch: {epoch}, iter: {i}, val_loss: {loss.item()}')
                pbar1.set_postfix(Epoch=epoch, val_loss=loss.item(), rmse_esti_ori=rmse, rmse_init_ori=ori_rmse, dot = dot)
                
        tot_val_loss = np.mean(val_loss)
        val_loss_per_epoch.append(tot_val_loss)
        print(f'epoch: {epoch} training loss: {tot_train_loss}, tr_rmse : {np.mean(train_rmse)}, extra_rmse_diff_mse_6')
