...
def train_pcpnet(opt):
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, (data, norms, init, diff) in enumerate(pbar, 0):
            ...
            target = torch.ones(norms.shape[0]).to(device)
            ...
            rmse = rms_angular_error(esti_norms, norms)
        ...
        print(f'epoch: {epoch} training loss: {tot_train_loss}, tr_rmse : {np.mean(train_rmse)}, diff_cos_6')
        print(f'epoch: {epoch} val loss: {tot_val_loss}, val_rmse : {np.mean(val_rmse)}, diff_cos_6')
